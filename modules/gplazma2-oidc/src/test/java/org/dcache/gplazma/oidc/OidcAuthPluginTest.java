...
public class OidcAuthPluginTest
{
    ...
    public void setUp() throws Exception
    {
        ...
        helper = Mockito.mock(HttpJsonHelper.class);
    }
    ...
    public void successWhenValidToken() throws Exception
    {
        ...
        assertThat(principals, hasNames("Kermit The", "Frog", "Kermit The Frog"));
    ...
    {
        OidcAuthPlugin plugin = new OidcAuthPlugin(givenConfiguration, helper, cache);

        Mockito.doReturn(discoveryDoc).when(cache).get(Mockito.anyString());
        Mockito.doReturn(userInfo).when(helper).doGetWithToken(Mockito.anyString(), Mockito.anyString());
            ...
            throws ExecutionException, IOException, AuthenticationException
    {
        ...
        Mockito.doReturn(discoveryDoc).when(cache).get(Mockito.anyString());
        Mockito.doThrow(e).when(helper).doGetWithToken(Mockito.anyString(), Mockito.anyString());
    ...
    public static Matcher<Iterable<? super ExternalNamesPrincipal>> hasNames(String givenName, String familyName, String fullName)
    {
        return hasItem(new ExternalNamesPrincipal(givenName, familyName, fullName));
    }
}