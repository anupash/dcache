...
public class GplazmaLocalMapperPluginTest {
    ...
    private void givenConfig(String map) throws IOException {
        final File localmapper = tempFolder.newFile("localmapper.conf");
        givenConfiguration.put("gplazma.localmapper.file", localmapper.getPath());

        FileUtils.writeStringToFile(localmapper, map);
    }


    private void whenMapPluginCalledWith(GplazmaLocalMapFile map, Set<Principal> principals)
            throws AuthenticationException
    {
        GplazmaLocalMapperPlugin plugin =  new GplazmaLocalMapperPlugin(map);
    ...
    private void whenMapPluginCalledWith(Properties properties, Set<Principal> principals)
            throws AuthenticationException
    {
        GplazmaLocalMapperPlugin plugin =  new GplazmaLocalMapperPlugin(properties);
        plugin.map(principals);
    }

    private GplazmaLocalMapFile mockedMapFileWithNoMapping() {
        GplazmaLocalMapFile map =  Mockito.mock(GplazmaLocalMapFile.class);
        doReturn(new HashSet<>()).when(map).getMappedPrincipals(Mockito.any());
        doNothing().when(map).refresh();
        return map;
    }


    private GplazmaLocalMapFile mockedMapFile() {
        GplazmaLocalMapFile map =  Mockito.mock(GplazmaLocalMapFile.class);
